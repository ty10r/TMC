    ...
    $.widget('ui.tmPriceSlider', $.ui.slider, {
        ...
        _create_price_data: function (prices) {
            if (!this.options.precalculated) {
                ...
                var values = this.options.calculated_values;
                this._min_price = values.min_price;
                this._max_price = values.max_price;
                this._display_min = this._calc_display_min(values.min_price);
                this._display_max = this._precalc_display_max();
            }
        ...
        _create_price_slider: function () {
            ...
            self._price_slider_widget = self.element.slider({
                ...
                create: function () {
                    ...
                    stop_v = start_v;
                    o.min.text(self.options.numberFormatter(self._display_min));
                    o.max.text(self.options.numberFormatter(self._display_max));
                ...
                slide: function (event, ui) {
                    ...
                    o.min.text(self.options.numberFormatter(ui.values[0]));
                    o.max.text(self.options.numberFormatter(ui.values[1]));
        ...
        _precalc_display_max: function () {
            var q1_median = this.options.calculated_values.q1_median;
            var cutoff = this._calc_cutoff(this.options.calculated_values.min_price, q1_median);
            var display_max = this.options.calculated_values.max_price;
            if (display_max > cutoff) {
                display_max = cutoff;
            }
            if (display_max % this.options.increment !== 0) {
        ...
        _update_price_slider: function (prev_price_range) {
            ...
            o.min.text(self.options.numberFormatter(value[0]));
            o.max.text(self.options.numberFormatter(value[1]));
        ...
        display_price_range: function () {
            return ([this._display_min, this._display_max]);
        },
