        ...
        $(form).request({
            ...
            onSuccess: function(t) {
                ...
                if( loc_data.redirect == 1 ) {
                    ...
                    $('location_input').addClassName('error');
...
var ISCLanding = {
    ...
    setup: function (id, kill) {
        ...
        if(id && url_type  && (url_type === 'c' || url_type === 'h')){
        ...
        if (id && id['type'] && id['type'] === "event_id") {
            ...
            if (landing && (landing[1] === 'c' || landing[1] === 'h')) {
...
LiveChatTemplate = {
    ...
    _handle_form_link: function() {
        ...
        if (this._parentShow){
            ...
            window.location = url_append_arg(document.URL, 'adads_id', 1);
        }
    },
...
function masbuilder( containerEL, listContainer, names, opts) {
    ...
    if( num_names == 1 ) {
    ...
    else if( num_names <= names.length ) {
        // attach "more" events
        ...
        if( !hadToShrink ) {
            namelistContainer.removeChild( tmp[tmp.length-1] );
            namelistContainer.removeChild( tmp[tmp.length-1] );
        }
    }

    var moreEl = namelistContainer.select('a');
    if( moreEl.length ) {
        moreEl = moreEl[ moreEl.length-1 ];
 ...
 * @return {String}
 */
TM.util.formatCurrency = function(value, options) {
  ...
  return (options.pre ? rules[2]||"" : "")
    + formatted
    + (options.decimal ? (rules[1]||"")+dec : "")
    + (options.post    ? (rules[3]||"") : "");
};
 ...
 * @param {Object}: setup
 * see ECOM dashboard for indepth documentation
 */
...
;(function( $, window, document, undefined ) {
    function TMTextMoreLess(elem, opts){
            ...
            this.txt = this.opts.contentText || this.opts.txt || $(this.elem)[this.contentType]() || false;
            this.txtCnt = $('<i>' + this.txt + '</i>').text().length;
            this.elemWidth = parseInt(this.opts.width, 10) || $(this.elem).width() || false;
            this.dataStore = {};
            this.setup = {
                more : { txt : '', toggleData: 'less', delimiter: this.opts.delimiter, expand: 'true' },
                less : { txt : this.txt, toggleData: 'more', delimiter: ' ', expand: 'false' },
