...
/**
 * A class, for creating popup elements.
 * @param {String} layer_id the element id of the container for the popup
 * @param {Object=} opt_obj An object containing config values (optional)
 *     The object can contain:
 *     links: {Array.<Object>} Array of object that defines the triggers of
 *         the popup and each trigger's associated behaviour.
 *         @see Popup#addLinks
 *     onHide: {Function} Function to call when popup hides. with
 *         'this' object of the options of this instance of Popup, or the
 *         combination of the options and Event object. This function can
 *         take 1 argument (the instance of Popup).
 *     carets: {
 *       auto: true|false // automatically populates caret with tm-top-caret and tm-bottom-caret class
 *                        // if used, this will override the top/bottom values
 *       top: Element     // top caret
 *       bottom: Element  // bottom caret
 *       gap: int         // pixel of caret from edge, default: 0
 *       style: Object    // if given, used for setStyle on top/bottom carets
 *     }
 * @constructor
 */
Popup = Class.create( {
    /**
     * @param {string} layer_id The HTMLDomElement ID of the popup container.
     */
...
/*
            ...
            hide_event: '' || [], // can be a string or an array (list of events)
*/
            ...
            link_event: 'click', // can be a string or an array (list of events)
        ...
        if (opts.carets && opts.carets.auto) {
          ...
          if (opts.carets.style) {
            opts.carets.top.setStyle(opts.carets.style);
            opts.carets.bottom.setStyle(opts.carets.style);
          }
    ...
    /**
     * Add triggers and defines behaviour for the pop up
     *
     * @param {Array.<Object>} links Triggers for the current popup. An array
     * of object that contains the key/value:
     *     link_id : {string} An HTMLDOMElement IDs. Invalid reference is
     *         skipped without errors.
     *     link_event: {(string|Array.<string>)} Type of event(s) that trigger
     *         the popup. Default is "click".
     *     hide_event: {(string|Array.<string>)=} Type of event(s) that trigger
     *         the popup to hide. Optional.
     *     autohide_timeout: xxx
     *     anchor_align: {{x: number, y: number}=} Defaults to. Optional.
     *     popup_align: {{x: number, y: number}=} Defaults to. Optional.
    ...
    // don't let popups be dragged off of the screen
    ...
    moveTo: function( x, y ) {
        this.layer.style.left = (isNaN(x)?0:x) + "px";
        this.layer.style.top = (isNaN(y)?0:y) + "px";
    ...
    is_shown: function () {
        return (this.layer.style.display === 'none') ? false : true;
    },
    ...
    update_position: function( anchor_options ) {
        ...
        if ( anchor ) {
            ...
            // subtract out container div scroll offsets
            ...
            do {
                ...
                // determines where the reference point is on the popup
                // and adjust offset
                if (anchor_options.offset_adjustment) {
                  var op = {x:0, y:0};

                  if ( popup_align.x > 0 )      { op.x = -1; }
                  else if ( popup_align.x < 0 ) { op.x =  1; }

                  if ( popup_align.y > 0 )      { op.y = -1; }
                  else if ( popup_align.y < 0 ) { op.y =  1; }

                  offset[0] = offset[0] + op.x * anchor_options.offset_adjustment.x;
                  offset[1] = offset[1] + op.y * anchor_options.offset_adjustment.y;
                }
                ...
                if ( anchor_options.carets ) {
                    ...
                    if ( anchor_options.carets[loc] ) {
                        ...
                        // need to show before trying to get the width
                        caret.show();
                ...
                if ( constrain ) {
                    ...
                    // we are constraining.  position: fixed only makes sense when constraining
                    fixed = false;
